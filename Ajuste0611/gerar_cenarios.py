...
CENARIOS_DIR = os.path.join(BASE_DIR, "cenarios")  
TLS_IDS = ["n00","n01","n02","n10","n11","n12","n20","n21","n22"]  #nome semaforo cfg

BASE_CONFIG = os.path.join(BASE_DIR, "baseSumo/grid.sumocfg")
BASE_NET    = os.path.join(BASE_DIR, "baseSumo/grid.net.xml")
BASE_ADD    = os.path.join(BASE_DIR, "baseSumo/grid.add.xml")
...
def criar_cenario(nome, carros_total, RL, A, F, tipo=None):
    ...
    for tripinfo in output_tag.findall("tripinfo-output"):
        output_tag.remove(tripinfo)
    
    tripinfo_tag = ET.SubElement(output_tag, "tripinfo-output")
    tripinfo_tag.set("value", "tripinfo.xml")
    tree.write(os.path.join(scenario_path, "grid.sumocfg"), encoding="utf-8", xml_declaration=True)
