...
BATCH_SIZE = 64
TARGET_UPDATE = 10
# fallback sizes (caso não seja possível inferir)
...
if __name__ == "__main__":
    ...
    # primeiro reset para obter observações iniciais (e confirmar obs spaces)
    observations, infos = env.reset()

    # criar um DQN por agente RL com state/action size corretos
    for agent_id in agentes_rl:
        # tenta extrair observation_space e action_space da API
        state_size = None
        action_size = None
        try:
        ...
        if state_size is None:
            # último recurso: usar len da observação se presente
            if agent_id in observations and observations[agent_id] is not None:
                try:
                    state_size = len(observations[agent_id])
                except Exception:
    ...
    # estado para agentes fixos (baseline alternante)
        ...
        while env.agents:
            ...
            for agent_id in agentes_rl:
                if agent_id in states:
                    ...
                    # safety clamp: garantir que action esteja dentro do action_space
                    try:
                        ...
                        # se não conseguir acessar action_space, mantenha action
                        pass

                    actions_dict[agent_id] = int(action)

            # não-RL: alterna 0/1 a cada step (você já fazia isso; envia dummy)
