...
class SumoEnv(gym.Env):
    ...
    def __init__(self, sumo_cfg, tls_config, use_gui=False, max_steps=4200, steps_por_decisao=10, sumo_binary=None):
        super().__init__()
        ...
        self.steps_por_decisao = steps_por_decisao
        self.current_step = 0
        self.sumo_binary = sumo_binary or sumolib.checkBinary("sumo")

        # Lê configuração dos TLS
        try:
    ...
    def _init_lane_maps(self):
        ...
        traci.start([self.sumo_binary, "-c", self.sumo_cfg], port=port, label="temp_init")
    ...
    def reset(self, seed=None, options=None):
        ...
        for tls_id in self.rl_ids:
            phase_index = self.agent_phase_data[tls_id]["current_phase"] * 2
    ...
    def step(self, actions):
        reward = {}
        done = {tls: False for tls in self.rl_ids}
        ...
        for _ in range(self.steps_por_decisao):
            self.current_step += 1
            self._handle_adaptive_tls()
            traci.simulationStep()

            for tls_id in self.rl_ids:
                reward[tls_id] = reward.get(tls_id, 0.0) + self._compute_agent_reward(tls_id)
...
##adaptativo aqui, so dei ctrl c ctrl v do outro, provavelmente está horrivel
        ...
        for tls_id in self.adaptive_ids:
            ...
            try:
                ...
                if congestion:
                    ...
                    if traci.trafficlight.getPhase(tls_id) != best and traci.trafficlight.getNextSwitch(tls_id) - traci.simulation.getTime() <= 2:
    ...
    def render(self):
        if not self.use_gui:
            print("Render chamado, mas GUI desativada.")
